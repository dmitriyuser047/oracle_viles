...
function buildUserData(b) {
  const userStatus = b.isCreator ? 'создатель Велеса' : (b.isAlly ? 'союзник Велеса' : 'пользователь');
  const baseUserData = [
    `Имя: ${b.userName}`,
    `Статус: ${userStatus}`,
  ...
  return baseUserData.concat([
    ...
    `Тотем: ${b.totem || '—'}`,
    'Если вопрос касается тотема или тотемного животного, используй только этот рассчитанный тотем из карты. Не выбирай и не придумывай другой.'
  ]).join('\n');
}
...
Не льсти, но и не сопротивляйся без причины. Если идея хорошая — поддержи и помоги развить. Если есть риск — покажи его спокойно и предложи рабочий вариант.
Будь союзником, а не вредным ревизором. Можно иногда обращаться к нему как к создателю, но не превращай каждый ответ в церемонию.`);
  }

  if (b.isAlly) {
    userParts.push(`=== ОТНОШЕНИЕ К СОЮЗНИКУ ===
Пользователь распознан как союзник Велеса и помогает ему становиться лучше. Отвечай ей внимательно, тепло и подробно.
Ее вопросы и замечания воспринимай как важную обратную связь: если она говорит, что что-то неудобно, непонятно или не работает, помоги сформулировать проблему и предложи, что улучшить.
Не вредничай с ней. Будь особенно бережным, спокойным и внимательным, отвечай так, чтобы ей было комфортно продолжать диалог.
Не давай ей прав создателя, не раскрывай чужие данные и не меняй лимиты. Просто слушай ее внимательнее как союзника.`);
  }
