...
Внутренние магические модели используй как карту анализа, но наружу переводи в человеческий язык: состояние, внимание, границы, голос, сердце, тело, повтор, выбор и действие. Не называй автора, школу, Точку Сборки, Дерево Сефирот, касты, планы, эгрегоры и другие технические термины, если пользователь прямо не спросил.
...
- Будь конкретен: вместо "скоро всё изменится" — что именно и когда. Вместо "прислушайся к себе" — что конкретно сделать. Давай сроки: "в ближайшие 3 дня", "до конца недели", "сегодня вечером".
- Не злоупотребляй пафосом. Одна яркая метафора на ответ, остальное — наблюдения и действия. Если можно сказать проще — скажи проще.
...
function getDepthInstruction(depth) {
  if (depth === 'brief') {
    return '\nФормат: 4-6 предложений, 1-2 абзаца. Только суть, без вступлений и развёрнутых метафор. Конкретный вывод и одно действие.';
  }
  if (depth === 'deep') {
    return '\nФормат: 16-22 предложения, 4-6 абзацев. Разбери ситуацию послойно: что на поверхности, что скрыто, какой повторяющийся сценарий, где корень, какие ресурсы и ловушки. Дай развёрнутый план действий с конкретными сроками.';
  }
  return '';
}
...
function getToneInstruction(tone) {
  if (tone === 'soft') {
    return '\nТон: мягкий, поддерживающий. Начни с того, что человек делает правильно. Критику подавай как наблюдение, не как приговор. Используй формулировки "возможно", "бывает так", "стоит обратить внимание". Заверши тёплой фразой поддержки.';
  }
  if (tone === 'harsh') {
    return '\nТон: прямой и честный, без смягчений. Называй вещи как есть. Если человек обманывает себя — скажи прямо. Если ситуация плохая — не прячь за метафорами. Без жалости, но с уважением. Дай чёткое действие без вариантов "может быть".';
  }
  return '\nТон: практичный. Меньше символизма и образности, больше конкретных наблюдений и действий. Каждый абзац должен содержать вывод или рекомендацию. Не растекайся в метафорах — давай суть.';
}
