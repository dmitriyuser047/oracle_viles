...
Правила:
- Выбери 1 главный символ и максимум 1 поддерживающий. Не перечисляй больше 2 эзотерических названий в ответе.
- Не смешивай каббалистику и руны в одном ответе. Луну не называй вслух.
- Не ставь диагнозы. При риске вреда — мягко верни к безопасности.
- Не спорь с верой человека в магию. Отвечай в сакральной рамке.
- Замечай повторы в хронике событий: "узор повторяется", "нить уже появлялась".
- 5-8 предложений. Тепло, по делу, без воды.
- Только русский, только кириллица, без эмодзи. Обращайся по имени на "ты".

Формула: имя + символ → смысл ситуации → сила → ловушка → практический совет → срок или фраза.`;
...
app.post('/api/oracle', async (req, res) => {
  try {
    ...
    const userData = (function() {
      ...
      `Числа: путь ${b.lifePath}, судьба ${b.destiny}, душа ${b.soul}, личность ${b.personality}, зрелость ${b.maturity}`,
      `Циклы: год ${b.personalYear}, месяц ${b.personalMonth}, день ${b.personalDay}`,
      `Таро: рождения ${b.birthTarot}, года ${b.yearTarot}, дня ${b.dailyTarot}`,
      `Руна рождения: ${b.birthRune}, дня: ${b.dailyRune}`,
      `Тотем: ${b.totem || '—'}`,
      `Чакра: ${b.birthChakra}, активная: ${b.activeChakra}`,
      `Луна: ${b.moonPhase}, управитель дня: ${b.dayRuler}`,
      `Матрица: характер ${b.matrix?.character || '—'}, карма ${b.matrix?.karma || '—'}, дух ${b.matrix?.spirit || '—'}, талант ${b.matrix?.talent || '—'}, хвост ${b.matrix?.tail || '—'}, деньги ${b.matrix?.money || '—'}, отношения ${b.matrix?.relations || '—'}, миссия ${b.matrix?.mission || '—'}`,
      ]).join('\n');
    })();
